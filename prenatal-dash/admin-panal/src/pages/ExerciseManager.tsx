...
const EMPTY_WEEK_ENTRY: Omit<ExerciseWeekEntry, 'id'> = {
    week: 18,
    trimester: '2nd',
    month: 5,
    isPublished: true,
...
export default function ExerciseManager() {
    ...
    const [formData, setFormData] = useState<ExerciseWeekEntry>({ id: '', ...EMPTY_WEEK_ENTRY });
    const [entryToDelete, setEntryToDelete] = useState<ExerciseWeekEntry | null>(null);
    ...
    const loadWeekGuidesData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await cmsClient.list<BackendExerciseRow>('exercise-weeks', { limit: 500 });
            setRawRows(res.items || []);
        } catch (err: any) {
            showToast(err.message || 'Failed to load exercise weeks', 'error');
        } finally {
            setLoading(false);
        }
    }, [showToast]);
   ...
   const weekEntriesList = useMemo(() => {
    ...
    const handleOpenEditModal = (entry: ExerciseWeekEntry) => {
        setFormData({ ...entry });
        setIsEditModalOpen(true);
    };

    const handleOpenDeleteModal = (entry: ExerciseWeekEntry) => {
  ...
  const handleSaveWeekGuide = async (e: React.FormEvent) => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await cmsClient.delete('exercise-weeks', entryToDelete.id);
            setIsDeleteModalOpen(false);
            setEntryToDelete(null);
            showToast(`Week ${entryToDelete.week} Exercise Guide deleted.`, 'success');
            loadWeekGuidesData();
        } catch (err: any) {
            showToast(err.message || 'Failed to delete exercise guide', 'error');
        }
    };

    // ── Toggle Publish Status ────────────────────────────────────────────────
    const handleTogglePublish = async (entry: ExerciseWeekEntry) => {
        try {
            const newStatus = !entry.isPublished;
            await cmsClient.update('exercise-weeks', entry.id, { isPublished: newStatus });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[#047857]">Exercise Week Guides</h2>
                    <p className="text-gray-500 text-sm mt-0.5">
                        Manage weekly maternal exercise importance, trimesters, and safe activity tips in 4 languages
                    </p>
                </div>
                <Button
                    onClick={handleOpenAddModal}
                    className="bg-[#047857] text-white hover:bg-[#065f46] text-sm px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm"
                >
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full text-xs rounded-xl pl-9 pr-3 py-2 border border-gray-200 bg-white focus:outline-none focus:border-[#047857]"
                            />
                        </div>
                    </div>
                </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center p-16 text-gray-400">
                    ...
                    <p className="text-sm font-medium">Loading exercise week guides...</p>
                </div>
            ) : filteredEntries.length === 0 ? (
                <div className="text-center p-12 bg-white rounded-2xl border border-gray-100 shadow-sm space-y-3">
                    <p className="text-sm text-gray-500 font-medium">No exercise week guides found matching your filters.</p>
                    <Button
                        onClick={handleOpenAddModal}
                        className="bg-[#047857] text-white hover:bg-[#065f46] text-xs px-4 py-2 rounded-xl font-bold"
                    >
                        Create Week Guide
                    </Button>
                </div>
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs text-gray-500 px-1">
                        <span>Showing <strong>{filteredEntries.length}</strong> exercise week guides</span>
                    </div>
                            ...
                            return (
                                <Card key={item.id} className="p-5 space-y-4 border border-gray-100 shadow-sm hover:shadow-md transition-all rounded-2xl bg-white flex flex-col justify-between">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-start justify-between gap-3 border-b border-gray-100 pb-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-2xl bg-emerald-50 border border-emerald-100 text-[#047857] flex flex-col items-center justify-center shrink-0 shadow-xs">
                                                    <span className="text-base font-extrabold leading-none">{item.week}</span>
                                                    <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-700">Week</span>
                                                </div>
                                                <div>
                                                    <h3 className="font-bold text-gray-900 text-base">{title || `Week ${item.week} Exercise Guide`}</h3>
                                        ...
                                        <div className="flex items-center gap-1 bg-gray-50 p-1 rounded-xl w-fit text-[11px] font-bold">
                                            {(['en', 'or', 'so', 'am'] as const).map(l => (
                                                <button
                                                    ...
                                                    className={`px-2.5 py-0.5 rounded-lg transition-all ${lang === l ? 'bg-white text-[#047857] shadow-xs' : 'text-gray-400 hover:text-gray-600'}`}
                                                >
                                        ...
                                        <div className="space-y-1.5 bg-emerald-50/50 p-3.5 rounded-xl border border-emerald-100">
                                            <h4 className="text-xs font-bold text-emerald-900 flex items-center gap-1.5">
                                                ...
                                                Exercise Tips ({lang.toUpperCase()})
                                            </h4>
                                            <p className="text-xs text-gray-700 leading-relaxed line-clamp-2">
                                                {tips || <span className="italic text-gray-400">No exercise tips provided for this language.</span>}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] text-gray-400 font-medium">
                                        ...
                                        <span className="text-[#047857] font-bold">Month {item.month}</span>
                                    </div>
                                </Card>
                            );
                        })}
            ...
            <Modal
                ...
                title={isEditModalOpen ? `Edit Week ${formData.week} Exercise Guide` : '+ Add Exercise Week Guide'}
                size="xl"
            >
                <form onSubmit={handleSaveWeekGuide} className="space-y-6 py-2 max-h-[80vh] overflow-y-auto pr-1">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <TextArea
                                    ...
                                    placeholder="Explain why safe physical exercise is essential during this week..."
                                    rows={3}
                            ...
                            <div>
                                ...
                                <TextArea
                                    ...
                                    placeholder="በዚህ ሳምንት የአካል ብቃት እንቅስቃሴ ለምን አስፈላጊ እንደሆነ ያብራሩ..."
                                    rows={3}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                            <div>
                                ...
                                <TextArea
                                    ...
                                    placeholder="Ibsa maaliif sochiin qaamaa torban kana keessatti barbaachisaa ta'e..."
                                    rows={3}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                            <div>
                                ...
                                <TextArea
                                    ...
                                    placeholder="Fahfaahin sababta jimicsigu muhiimka u yahay todobaadkan..."
                                    rows={3}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                    ...
                    {/* SECTION 4: EXERCISE TIPS (4 LANGUAGES) */}
                    <div className="space-y-3 bg-emerald-50/40 p-4 rounded-2xl border border-emerald-100">
                        <h4 className="text-xs font-bold text-emerald-900 uppercase tracking-wider flex items-center gap-2">
                            ...
                            4. Exercise Tips (4 Languages)
                        </h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label className="block text-[11px] font-bold text-gray-600 mb-1">🇬🇧 English Exercise Tips</label>
                                <TextArea
                                    value={formData.tipsEn}
                                    onChange={e => setFormData(prev => ({ ...prev, tipsEn: e.target.value }))}
                                    placeholder="e.g. 20-30 min brisk walking, gentle pelvic floor stretches, maintain hydration..."
                                    rows={2}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                            <div>
                                <label className="block text-[11px] font-bold text-gray-600 mb-1">🇪🇹 Amharic Exercise Tips</label>
                                <TextArea
                                    value={formData.tipsAm}
                                    onChange={e => setFormData(prev => ({ ...prev, tipsAm: e.target.value }))}
                                    placeholder="ለምሳሌ፡ ከ20-30 ደቂቃ የእግር ጉዞ፣ የማህፀን አካባቢ እንቅስቃሴዎች..."
                                    rows={2}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                            <div>
                                <label className="block text-[11px] font-bold text-gray-600 mb-1">🌳 Afaan Oromo Exercise Tips</label>
                                <TextArea
                                    value={formData.tipsOr}
                                    onChange={e => setFormData(prev => ({ ...prev, tipsOr: e.target.value }))}
                                    placeholder="e.g. Daqiiqaa 20-30 deemuu, shaakala salphaa taasisuu..."
                                    rows={2}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                            <div>
                                <label className="block text-[11px] font-bold text-gray-600 mb-1">🇸🇴 Afan Somali Exercise Tips</label>
                                <TextArea
                                    value={formData.tipsSo}
                                    onChange={e => setFormData(prev => ({ ...prev, tipsSo: e.target.value }))}
                                    placeholder="e.g. 20-30 daqiiqo oo socod ah, jimicsi fudud oo jidhka ah..."
                                    rows={2}
                                    className="text-xs rounded-xl"
                                />
                            </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-200/60">
                        <div>
                            ...
                            <span className="text-[11px] text-gray-500">Make this exercise guide visible on the mobile app</span>
                        </div>
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                className="w-4 h-4 text-[#047857] rounded border-gray-300 focus:ring-[#047857]"
                            />
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <Button
                            type="submit"
                            disabled={submitting}
                            className="bg-[#047857] text-white hover:bg-[#065f46] text-xs px-6 py-2 rounded-xl font-bold flex items-center gap-2"
                        >
            ...
            <Modal
                ...
                title="Delete Exercise Week Guide"
            >
                    ...
                    <div className="flex items-center gap-3 bg-red-50 p-4 rounded-xl text-red-700 border border-red-100">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-red-600 mt-0.5">
                                Exercise Guide for Week {entryToDelete?.week} will be permanently removed.
                            </p>
                        </div>
                    </div>
            ...
            >
                <div className="p-6 text-center space-y-4">
                    ...
                    <Button
                        ...
                        className="bg-[#047857] text-white hover:bg-[#065f46] text-xs px-6 py-2 rounded-xl font-bold mx-auto"
                    >
                        Done
                    </Button>
                </div>
